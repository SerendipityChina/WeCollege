...
Page({
	...
	_loadDetail: async function (isDel) {
		...
		let data = await cloudHelper.callCloudData('admin/lost_data_get', params, options);
	...
	bindOpenTap: function (e) {
		fileHelper.openDoc('失物招领记录', this.data.url);
	},
	...
	bindExportTap: async function (e) {
		...
		try {
			...
			await cloudHelper.callCloudData('admin/lost_data_export', params, options).then(res => {
				this._loadDetail(0);
	...
	bindDelTap: async function (e) {
		try {
			...
			await cloudHelper.callCloudData('admin/lost_data_del', {}, options).then(res => {
				this.setData({
					url: '',
				});
