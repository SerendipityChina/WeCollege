...
Page({
	...
	_loadDetail: async function (isDel) {
		...
		let data = await cloudHelper.callCloudData('admin/leave_data_get', params, options);
	...
	bindOpenTap: function (e) {
		fileHelper.openDoc('闲置物品记录', this.data.url);
	},
	...
	bindExportTap: async function (e) {
		...
		try {
			...
			let params = {
				fields: projectSetting.LEAVE_FIELDS,
				status: this.data.formStatus,
				start,
				end,
			}

			await cloudHelper.callCloudData('admin/leave_data_export', params, options).then(res => {
				this._loadDetail(0);
	...
	bindDelTap: async function (e) {
		try {
			...
			await cloudHelper.callCloudData('admin/leave_data_del', {}, options).then(res => {
				this.setData({
					url: '',
				});
