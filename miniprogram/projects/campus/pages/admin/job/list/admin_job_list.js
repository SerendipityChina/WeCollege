...
Page({
	...
	_setSort: async function (e) {
		...
		try {
			await cloudHelper.callCloudSumbit('admin/job_sort', params).then(res => {
				pageHelper.modifyListNode(id, this.data.dataList.list, 'JOB_ORDER', sort);
				this.setData({
	...
	_del: async function (e) {
		...
		let callback = async () => {
			try {
				...
				await cloudHelper.callCloudSumbit('admin/job_del', params, opts).then(res => {
	...
	bindMoreTap: async function (e) {
		...
		let order = this.data.dataList.list[idx].JOB_ORDER;
			...
			success: async res => {
				switch (res.tapIndex) {
					case 0: { //预览
						let id = pageHelper.dataset(e, 'id');
						wx.navigateTo({
							url: '../../../job/detail/job_detail?id=' + id,
						});
						break;
					}
					case 1: { //置顶 
	...
	_setStatus: async function (e) {
		...
		try {
			await cloudHelper.callCloudSumbit('admin/job_status', params).then(res => {
				pageHelper.modifyListNode(id, this.data.dataList.list, 'JOB_STATUS', status, '_id');
				this.setData({
					dataList: this.data.dataList
				});
				pageHelper.showSuccToast('设置成功');
	...
	_getSearchMenu: function () {
		...
		let sortMenus = [
			...
			{ label: '点赞数ˇ', type: 'sort', value: 'JOB_LIKE_CNT|desc' },
			{ label: '浏览数ˇ', type: 'sort', value: 'JOB_VIEW_CNT|desc' },
			{ label: '收藏数ˇ', type: 'sort', value: 'JOB_FAV_CNT|desc' },
